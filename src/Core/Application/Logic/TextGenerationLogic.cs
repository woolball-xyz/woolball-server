...
public sealed class TextGenerationLogic : ITextGenerationLogic
{
    ...
    public async Task ProcessTaskResponseAsync(TaskResponse taskResponse, TaskRequest taskRequest)
    {
        try
        {
            Console.WriteLine($"[TextGenerationLogic] Processing text generation task: {taskRequest.Id}");
            ...
            Console.WriteLine($"[TextGenerationLogic] Response data type: {responseData.GetType().FullName}");
            Console.WriteLine($"[TextGenerationLogic] Response object type: {responseData.Response?.GetType().FullName ?? "null"}");
            ...
            if (responseData.Response == null)
            {
                ...
                await subscriber.PublishAsync(
                    resultQueueName,
                    JsonSerializer.Serialize(new GenerationResponse { 
                        GeneratedText = ""
                    })
                );
            ...
            try 
            {
                ...
                else
                {
                    ...
                    generationResponse = JsonSerializer.Deserialize<GenerationResponse>(jsonStr) 
                        ?? new GenerationResponse { GeneratedText = "" };
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[TextGenerationLogic] Error extracting GenerationResponse: {ex.Message}");
            ...
            Console.WriteLine($"[TextGenerationLogic] Serialized data (first 100 chars): {serializedData.Substring(0, Math.Min(100, serializedData.Length))}...");
            ...
            Console.WriteLine($"[TextGenerationLogic] Successfully published text generation result to {resultQueueName}");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[TextGenerationLogic] Error processing text generation: {ex.Message}");
            ...
            {
                Console.WriteLine($"[TextGenerationLogic] Failed to send error response: {innerEx.Message}");
            }
